...
class Config(object):
    ...
    def __init__(self, source_booru, source_api_key, target_api_key, target_booru,\
                 use_reverse = True, tag_mapping = None, source_filter_id = None,\
                 add_text = True, url_conversion = None):
        ...
        if url_conversion is None:
            self.url_conversion = self.source_booru
        else: 
            self.url_conversion = url_conversion
...
def dict_to_config(d) -> Config:
        ...
        url_conversion = d.get("url_conversion")
...
def get_img_link(image: dict, config: Config, alt_source = False):
    image_id = image["id"]
    if alt_source:
        booru = config.url_conversion
    else:
        booru = config.source_booru
    return f"https://{booru}/{image_id}"

def change_description(image: dict, config: Config):
    assert type(image) == dict
    description = image["description"]
    image_id = image["id"]
    new_description = sub(image_link_pattern, lambda m: replace_image_link(m, config.url_conversion), description)
    ...
    if config.add_text:
        ...
        if config.url_conversion != config.source_booru:
            import_text = import_text + f"\n\"Alt Link\":{get_img_link(image, config, True)} (Warning: May link to something unrelated)"
        if new_description == "":
